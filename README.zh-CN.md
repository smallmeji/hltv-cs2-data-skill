...
- 查看不同地图胜率、对位数据、选手近期状态，以及可用时的警匪分边比分。
...
默认轻量模式只读取当前会话里可访问的公开 HLTV 页面。如果配置了维护好的数据 API / 数据仓，可以提供更完整、更可复现的历史数据，但这不是安装本 skill 的前提。
...
它适合只安装 skill、没有私有 API、没有本地数据库、没有爬虫服务的用户。模型会读取当前会话里可访问的公开 HLTV 页面，并尽量输出可用的数据包。

Direct mode 的限制：

- 有些 HLTV 页面可能无法访问。
- 赛事 player rating 可能需要用户额外提供 HLTV stats 链接。
- 赛前阵容不一定可见。
